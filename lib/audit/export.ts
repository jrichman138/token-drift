...
// text exports so the wording stays in one place.
...
export function toHeadline(result: AuditResult, url: string): string {
...
export function toMarkdown(result: AuditResult, url: string): string {
  const lines: string[] = [];
  ...
  if (result.unusedTokens.length === 0) {
    lines.push('None — every token is in use on this page.');
  } else {
...
// Structured JSON export. Deterministic (no timestamp) so it is easy to diff and
// test; includes the orphan locations a developer needs to act on.
export function toJson(result: AuditResult, url: string): string {
  const payload = {
    url,
